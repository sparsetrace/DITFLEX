"""
sample_amap.py — render a 4x4 (16-image) grid from an AMAP checkpoint.

    modal run AMAP/sample_amap.py                                  # latest ckpt
    modal run AMAP/sample_amap.py --step 30000                     # a specific step
    modal run AMAP/sample_amap.py --step base --weights model      # base + AMAP, un-finetuned
    modal run AMAP/sample_amap.py --repo sparsetrace/AMAP --weights ema

Loads SiT-XL/2 + AMAP (flags read from the checkpoint's amap_config.json),
...
def sample(repo: str, step: str, weights: str, sample_steps: int,
    ...
    model, info = C.load_amap_checkpoint(dev, repo, step=step, weights=weights)
