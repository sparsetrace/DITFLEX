"""
Shared DMAP helpers used by both DMAP.py (finetune) and sample_dmap.py (sampler).
...
Checkpoint layout in the Hub repo (e.g. jcandane/DMAP):
    ...
    checkpoints/step_0010000/dmap_config.json     # step + DMAP flags + base
    samples/dmap_*.png
"""
...
# Fixed classes/seed identical to ditflex sampling/modal_sample.py so DMAP
...
def make_dmap_config(cfg_dict: dict):
    """DMAPConfig from a plain dict (checkpoint's dmap_config.json)."""
    from dmap_attention import DMAPConfig
    return DMAPConfig(
...
def fetch_checkpoint(repo: str, step: int):
    ...
    cfg = None
    for name in ("dmap_config.json", "amap_config.json"):
        try:
