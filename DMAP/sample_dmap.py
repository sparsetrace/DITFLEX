"""
sample_dmap.py — render a 4x4 (16-image) grid from a DMAP checkpoint.

    modal run DMAP/sample_dmap.py                                  # latest ckpt
    modal run DMAP/sample_dmap.py --step 30000                     # a specific step
    modal run DMAP/sample_dmap.py --step base --weights model      # base + DMAP, un-finetuned
    modal run DMAP/sample_dmap.py --repo jcandane/DMAP --weights ema

Loads SiT-XL/2 + DMAP (flags read from the checkpoint's amap_config.json),
...
def sample(repo: str, step: str, weights: str, sample_steps: int,
    ...
    model, info = C.load_dmap_checkpoint(dev, repo, step=step, weights=weights)
