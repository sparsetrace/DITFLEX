"""
Shared HMAP helpers used by both HMAP.py (finetune) and sample_hmap.py (sampler).
...
Checkpoint layout in the Hub repo (e.g. sparsetrace/HMAP):
    ...
    checkpoints/step_0010000/hmap_config.json     # step + HMAP flags + base
    samples/hmap_*.png
"""
...
# Fixed classes/seed identical to ditflex sampling/modal_sample.py so HMAP
...
def make_hmap_config(cfg_dict: dict):
    """HMAPConfig from a plain dict (checkpoint's hmap_config.json)."""
    from hmap_attention import HMAPConfig
    return HMAPConfig(
...
def resolve_checkpoint_step(repo: str, step) -> int | None:
    ...
    string, 'latest', or 'base'. 'base'/None -> None (un-finetuned base+HMAP)."""
...
def load_hmap_checkpoint(dev, repo: str, step="latest", weights: str = "ema"):
    """Build SiT-XL/2 + HMAP and load a finetuned checkpoint (or base+HMAP if
    ...
    if resolved is None:
        apply_hmap(model, HMAPConfig(), alpha=0.0)   # α=0 = AMAP endpoint
        return model, {"repo": repo, "step": "base", "weights": "base", "alpha": 0.0}

    folder = f"checkpoints/step_{resolved:07d}"
    cfg_dict = json.load(open(hf_hub_download(repo, f"{folder}/hmap_config.json")))
    apply_hmap(model, make_hmap_config(cfg_dict))   # before load (adds params if any)
    ...
    # set the homotopy coefficient this checkpoint was saved at (1 => exact/DMAP-class)
