"""
Shared FMAP helpers used by both FMAP.py (finetune) and sample_fmap.py (sampler).
...
Checkpoint layout in the Hub repo (e.g. sparsetrace/FMAP):
    ...
    checkpoints/step_0010000/fmap_config.json     # step + FMAP flags + base
    samples/fmap_*.png
"""
...
# Fixed classes/seed identical to ditflex sampling/modal_sample.py so FMAP
...
def make_fmap_config(cfg_dict: dict):
    """FMAPConfig from a plain dict (checkpoint's fmap_config.json)."""
    from fmap_attention import FMAPConfig
    return FMAPConfig(
