"""
sample_ddmap.py — render a 4x4 (16-image) grid from an DDMAP checkpoint.

    modal run DDMAP/sample_ddmap.py                                  # latest ckpt
    modal run DDMAP/sample_ddmap.py --step 30000                     # a specific step
    modal run DDMAP/sample_ddmap.py --step base --weights model      # base + DDMAP, un-finetuned
    modal run DDMAP/sample_ddmap.py --repo jcandane/DDMAP --weights ema

Loads SiT-XL/2 + DDMAP (flags read from the checkpoint's ddmap_config.json),
...
and writes a 4x4 grid PNG that the workflow commits into DDMAP/samples/.

Sampling is forward-only, so an L4 is plenty (default). The image matches
DDMAP.py so Modal reuses the cached build.
"""
...
def sample(repo: str, step: str, weights: str, sample_steps: int,
    ...
    model, info = C.load_ddmap_checkpoint(dev, repo, step=step, weights=weights)
